...
## 🎨 Phase H: Character Art Pipeline — ✅ COMPLETE
...
1. **`tools/compose.py` (Python + Pillow):** reads `tools/roster.json` (rank → LPC layer selections, keyed by the game's real rank names — Champion/Warlord/.../Relic, not generic Stratego names) and the LPC generator checkout's `sheet_definitions`/`spritesheets`, alpha-composites in JSON-declared z-order, handles both of LPC's color mechanisms (literal per-color folders and palette-swap recoloring, the latter via nearest-color match — some shipped LPC assets drift a few RGB units off their documented palette hex). Emits `assets/manifest.json` and auto-appends an LPC credits block to `CREDITS.md` (between `<!-- LPC-CREDITS:BEGIN/END -->` markers, never touching the hand-written sections above it). Deterministic: re-running with unchanged inputs reproduces byte-identical output. Also handles `"source": "static_prop"` entries (single-image, non-LPC props): thumbnails an arbitrary source image into the 64×64 frame and folds its `prop_credit` into a separate "Static Props" block in `CREDITS.md`.
2. **Composited:** all 10 mobile ranks × 2 teams (20 sheets, `assets/characters/`), plus `Ward` and `Relic` as static props (`assets/props/ward.png`, `assets/props/relic.png`) — a locked treasure chest (Movicx, CC-BY 4.0) and a cut diamond (7Soul1, curated CC0 by AntumDeluge), sourced from OpenGameArt since neither piece is an LPC character. All 12 ranks now have art.
3. **Engine wiring:** `piece.tscn` gained a `Visuals/AnimatedIcon` (`AnimatedSprite2D`) alongside the original `Icon` (`Sprite2D`, kept — handles `BACK_ICON` for hidden pieces and now also renders the static Ward/Relic props, since those are never `walkable` and so never get a `SpriteFrames`). `GameManager.get_sprite_frames(type, team)` lazily builds/caches a `SpriteFrames` from the manifest; `piece.gd::play_walk(dir)`/`play_idle()` are no-ops whenever the piece is showing its hidden back-icon, so `main.gd::_execute_move()` can call them unconditionally around its existing bump/land/slide tweens. **Gotcha fixed:** the `Icon` Sprite2D's scale (`0.082`) and `ICON_DARK` near-black tint were tuned for the old large flat-SVG icons; applying them unmodified to the new native-64×64 full-color prop art rendered Ward/Relic as an invisible speck. `piece.gd::_update_visuals()` now branches on `data.type in ["Ward", "Relic"]` to use `scale = Vector2.ONE` and no tint for prop art, `ICON_SVG_SCALE`/`ICON_DARK` for any legacy SVG fallback.
4. **Validated live:** `godot --path . -- --screenshot --autodeploy --aitest` in both Classic and Blitz, plus `--reveal` and `--defeat` — all 12 ranks render correctly with team coloring at both board sizes, hidden enemy pieces show only the plain back-icon (no animation leak). All `--rulestest` cases pass.
5. **Not yet done:** graduating the approved `docs/asset-plan.md` content into a committed root-level `PIPELINE.md` with one-command regen instructions (still local-only in `docs/`).
